# -*- coding: utf-8 -*-
"""Generative AI.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1GZjYD-L3Jl8aNBcA_GtlvfXRQBhtfXqw

LLAMA INDEX 2
"""

!pip install pypdf

!pip install transformers
!pip install einops
!pip install langchain
!pip install accelerate
!pip install bitsandbytes

!pip install install sentence_transformers

!pip install llama_index

from llama_index import VectorStoreIndex,SimpleDirectoryReader,ServiceContext
from llama_index.llms import HuggingFaceLLM
from llama_index.prompts.prompts import SimpleInputPrompt

from google.colab import drive
drive.mount('/content/drive')

documents = SimpleDirectoryReader("/content/drive/MyDrive/data").load_data()
documents

system_prompt="""
You are a Q&A assistant.Your goal is to answer as
accurately as possible based in the instructions and contex provided.
"""
## format for llama2
query_wrapper_prompt=SimpleInputPrompt("<|USER|>{query_str}<|ASSISTANT|>")

!!huggingface-cli login

!pip show accelerate
!pip show bitsandbytes

try:
    import accelerate
    accelerate_available = True
except ImportError:
    accelerate_available = False

print("Accelerate available:", accelerate_available)

try:
    import bitsandbytes
    bitsandbytes_available = True
except ImportError:
    bitsandbytes_available = False

print("Bitsandbytes available:", bitsandbytes_available)

import torch
llm = HuggingFaceLLM(
    context_window=4096,
    max_new_tokens=256,
    generate_kwargs={"temperature": 0.0, "do_sample": False},
    system_prompt=system_prompt,
    query_wrapper_prompt=query_wrapper_prompt,
    tokenizer_name="meta-llama/Llama-2-7b-chat-hf",
    model_name="meta-llama/Llama-2-7b-chat-hf",
    device_map="auto",
    # uncomment this if using CUDA to reduce memory usage
    model_kwargs={"torch_dtype": torch.float16, "load_in_8bit":True}
)

from langchain.embeddings.huggingface import HuggingFaceBgeEmbeddings
from llama_index import ServiceContext
from llama_index.embeddings import LangchainEmbedding

embd_model=LangchainEmbedding(
    HuggingFaceBgeEmbeddings(model_name="sentence-transformers/all-mpnet-base-v2"))

service_context = ServiceContext.from_defaults(
    chunk_size=1024,
    llm=llm,
    embed_model=embd_model
)

service_context

index = VectorStoreIndex.from_documents(documents,service_context=service_context)

index

query_engine=index.as_query_engine()

response=query_engine.query("who is the chef minister of tamilnadu?")
print(response)

